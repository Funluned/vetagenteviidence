...
VetResearch Workbench v0.3 增量阶段：首批科研正确性优化、网络药理学与分子对接准备闭环已完成本地实现与验收。
...
- [x] 快照升级到 schema v5；旧快照迁移时按新规则重建文献条件，并让不再可信的旧评估与报告失效

### 验证证据

- 全量自动测试：`131 passed`。
- 浏览器实际验收六步页面：问题与假设、文献证据、实验数据、机制预测、决策报告、运行记录。
- 浏览器加载合成网络演示后显示 2 个输入化合物、1 个目标物种、2 个交集靶点和 2 条通路；所有合成编号均明确标记为不可作为科研证据。
- 浏览器完成合成 RIS、范围匹配的 FICI 分析和报告生成；报告单列“计算预测（不等同于实验或直接文献证据）”，页面无未处理异常。
- 单元测试覆盖网络表必填列、物种/靶点连接、来源哈希、范围错配、Vina 清单、任务哈希与版本一致性、标准输出解析及禁止凭空生成 docking 分数。

## 阻塞

- 本机未安装 Docker：Dockerfile 已写，镜像构建未验证。
- 本机未安装 AutoDock Vina，也未提供真实配体、受体和运行输出，因此尚未执行真实分子对接；当前只完成可追溯的准备清单和用户输出的格式、版本、任务哈希校验，不能认证文件确由 Vina 实际运行产生。
- 尚未录制演示视频，也未获准进行公开部署。
