...
v0.2 的多轮检索在 PMID 去重后继续调用上述分区和规则提取能力，没有另建第二套检索系统。
...
| `workbench_pipeline.py` | v0.2 | 多轮检索、统一文献、条件矩阵、评估与决策报告 |
...
- 多轮检索最多 3 个查询，并以 PMID 去重后限制保留数量；
...
- 运行快照可序列化、原子保存并恢复；
- 合成演示数据在输入和报告中明确标记；
- 全量自动化回归当前为 `69 passed`。

真实验收执行 3 轮 NCBI 检索，按 PMID 去重后得到 5 个唯一结果。外部 PubMed 数据会更新，该数量只代表当前验收。
